...
# Must match the run_ids entry evaluated in config/evaluate/eval_test_agg_sample.yml
RUN_ID=era5_1deg_daily96h

uv run --directory /home/sagemaker-user/git/WeatherGenerator python scripts/monitor_resources.py \
  --interval 2 \
  --output "$WEATHERGEN_PRIVATE_REPO_PATH/results/${RUN_ID}/resource_usage_evaluate_agg_sample.csv" \
  -- \
  uv run --directory /home/sagemaker-user/git/WeatherGenerator evaluate \
    --config=config/evaluate/eval_test_agg_sample.yml
